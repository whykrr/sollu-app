...
const handleIncomingNotification = notification => {
    ...
    const message =
        notification.message || notification.data?.message || 'Anda memiliki pemberitahuan baru.'
...
const setupEchoListener = () => {
    ...
    notificationChannel = window.Echo.private(`App.Models.User.${userId}`).notification(
        notification => {
...
onMounted(() => {
    ...
    window.triggerTestToast = (
        title = 'Notifikasi Baru',
        message = 'Ini adalah contoh pesan notifikasi toast realtime.',
        type = 'info'
    ) => {
