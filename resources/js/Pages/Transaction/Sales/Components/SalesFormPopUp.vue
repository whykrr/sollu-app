    ...
    <div class="space-y-4 pb-24">
        ...
        <div class="space-y-4">
            <h3
                ...
                Informasi Dokumen</h3>
            <div class="grid grid-cols-1 gap-2">
                <AsyncOutletDropdown v-model="form.outlet_id"
                                     label="Outlet"
                ...
                <div>
                    <label
                           class="block text-sm font-medium mb-1">Pelanggan</label>
            ...
            <div class="grid grid-cols-2 gap-2">
                <DropdownField v-model="form.channel"
            ...
            <div class="grid grid-cols-2 gap-2">
                <DropdownField v-model="form.payment_term"
                               label="Metode Pembayaran"
                               :options="[{ value: 'tunai', label: 'Tunai' }, { value: 'termin', label: 'Kredit / Termin' }]"
                               :error="form.errors.payment_term"
                               required />

                <TextField v-if="form.payment_term === 'termin'"
                           type="date"
        ...
        <div class="space-y-4">
            <div class="flex justify-between items-center mb-2">
                ...
                <div class="w-72">
                    <AsyncSelectField id="search_product"
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-gray-500">
                                        ...
                                        <span v-if="item.track_inventory" class="ml-2">
                                            Stok: <span :class="item.current_stock > 0 ? 'text-gray-700 font-medium' : 'text-rose-600 font-semibold'">{{ item.current_stock }}</span>
                                        </span>
                ...
                <div v-for="(item, index) in form.items"
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <div v-if="item.promo_name" class="text-xs text-emerald-600 font-medium mt-0.5">
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <NumberField v-model="item.price"
                                     label="Harga"
                                     prefix="Rp"
                                     @update:modelValue="calculateTotals" />
                        <NumberField v-model="item.qty"
                                     label="Kuantitas"
                                     @update:modelValue="calculateTotals" />
                        <NumberField v-model="item.discount_amount"
                                     label="Diskon (Rp)"
                                     prefix="Rp"
                                     :disabled="!can('transaction.discount_manual')"
                                     @update:modelValue="calculateTotals" />
                        <NumberField v-model="item.subtotal"
                                     label="Subtotal"
                                     prefix="Rp"
                                     disabled />
                    </div>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    <AsyncSelectField id="promo_search"
                                      label="Cari Promo"
                            ...
                            <div class="flex justify-between items-center w-full">
                                <div>
                                    ...
                                    <div class="text-xs text-slate-500">
                                        {{ item.target_type === 'bill' ? 'Per Total Bill' : 'Per Produk' }} -
                                        {{ item.promo_type === 'percentage' ? item.discount_value + '%' : formatCurrency(item.discount_value) }}
                                    </div>
                                </div>
                            </div>
                        </template>
                    </AsyncSelectField>
                    <div v-if="selectedPromo" class="flex items-center justify-between text-xs text-emerald-700 bg-emerald-50 p-2 rounded-lg border border-emerald-200 mt-1">
                        <span>Promo aktif: <strong>{{ selectedPromo.name }}</strong></span>
                        <button type="button" @click="clearPromo" class="text-rose-600 font-semibold hover:underline">Batalkan Promo</button>
                    </div>
                </div>
                <div>
                    <NumberField v-model="form.manual_discount_amount"
        ...
        <div
            ...
            <div class="flex justify-between">
                <span class="text-slate-500">Biaya
                    Pengiriman</span>
                <div class="w-32">
                    <NumberField v-model="form.shipping_fee"
                                 @update:modelValue="calculateTotals"
                                 class="!py-1" />
                </div>
            </div>
            <div class="flex justify-between">
                ...
                <div class="w-32">
                    <NumberField v-model="form.tax_amount"
                                 @update:modelValue="calculateTotals"
                                 class="!py-1" />
                </div>
            </div>
            <div class="flex justify-between">
                <span
                    ...
                    Charge</span>
                <div class="w-32">
                    <NumberField v-model="form.service_charge_amount"
                                 @update:modelValue="calculateTotals"
                                 class="!py-1" />
                </div>
            </div>
        ...
        <div class="space-y-4">
            ...
            <TextareaField v-model="form.terms_and_conditions"
        ...
        <Teleport v-if="isMounted"
            ...
            <div
                ...
                <button type="button"
                        class="btn btn-flat"
                        @click="popUpStore.close()">Batal</button>
...
const calculateTotals = () => {
    ...
    form.discount_amount = Number(form.manual_discount_amount || 0) + Number(form.promo_discount_amount || 0);

    let total = subtotal - form.discount_amount
...
const addProductToItems = (product, promo = null) => {
    const targetProductId = product.product_id || product.id;
    const existingIndex = form.items.findIndex(i => i.product_id === targetProductId);
