    ...
    <Container>
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <ContainerHeader title="Detail Transaksi" />
            </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <span class="font-medium">{{ formatDateTimeSimple(transaction.created_at) }}</span>
                        </div>
                        <div>
                            <span class="text-sm text-gray-500 block">Pelanggan</span>
                            <span class="font-medium">{{ transaction.customer?.name || 'Guest' }}</span>
                        </div>
                        <div>
                            <span class="text-sm text-gray-500 block">Kasir / Shift</span>
                            <span class="font-medium">{{ transaction.shift?.user?.name || '-' }}</span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <span class="font-medium uppercase">{{ transaction.payment_status }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody>
                                <tr v-for="item in transaction.items" :key="item.id" class="border-b border-gray-100 last:border-0">
                                    <td class="p-3">
                                        <div class="font-medium">{{ item.product_name }}</div>
                                        <div v-if="item.modifiers && item.modifiers.length" class="text-xs text-gray-500 mt-1">
                                            <span v-for="(mod, idx) in item.modifiers" :key="mod.id">
                                                {{ mod.modifier_name }} <span v-if="idx < item.modifiers.length - 1">, </span>
                                            </span>
                                        </div>
                                    </td>
            ...
            <div class="col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex justify-between" v-if="Number(transaction.discount_amount) > 0">
                            <span class="text-gray-500">Diskon</span>
                            <span class="text-danger">-{{ formatCurrency(transaction.discount_amount) }}</span>
                        </div>
                        <div class="flex justify-between" v-if="Number(transaction.service_charge_amount) > 0">
                            <span class="text-gray-500">Service Charge</span>
                            <span>{{ formatCurrency(transaction.service_charge_amount) }}</span>
                        </div>
                        <div class="flex justify-between" v-if="Number(transaction.tax_amount) > 0">
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div v-if="transaction.payments && transaction.payments.length > 0" class="space-y-3 text-sm">
                        <div v-for="payment in transaction.payments" :key="payment.id" class="flex justify-between items-start pb-3 border-b border-gray-100 last:border-0 last:pb-0">
                            <div>
                                <div class="font-medium">{{ payment.payment_method?.name || 'Unknown' }}</div>
                                <div class="text-xs text-gray-500">{{ formatDateTimeSimple(payment.created_at) }}</div>
                            </div>
                            <div class="text-right">
                                <div class="font-medium">{{ formatCurrency(payment.amount) }}</div>
                                <div v-if="Number(payment.change_amount) > 0" class="text-xs text-gray-500">Kembalian: {{ formatCurrency(payment.change_amount) }}</div>
                            </div>
                        </div>
                    </div>
