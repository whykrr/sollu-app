        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-12">
            ...
            <div class="lg:col-span-7 flex flex-col gap-6">
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-xs p-5">
                    ...
                    <div class="flex flex-col gap-4">
                        <div>
                            <label class="form-label text-sm font-medium text-slate-700 mb-1.5 block">
                                Ukuran Kertas Thermal
                            </label>
                            <div class="grid grid-cols-2 gap-3">
                                <label
                                    class="flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all"
                                    :class="form.paper_size === '58mm' ? 'border-main bg-main/5 text-main font-semibold' : 'border-slate-200 hover:bg-slate-50 text-slate-700'"
                                >
                                    <div class="flex items-center gap-2">
                                        <input
                                            ...
                                            type="radio"
                                            value="58mm"
                                            class="text-main focus:ring-main"
                                        />
                                        <span>58 mm (Standar)</span>
                                    </div>
                                    <span class="text-xs text-slate-400 font-normal">~32 Karakter</span>
                                </label>
                                <label
                                    class="flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all"
                                    :class="form.paper_size === '80mm' ? 'border-main bg-main/5 text-main font-semibold' : 'border-slate-200 hover:bg-slate-50 text-slate-700'"
                                >
                                    <div class="flex items-center gap-2">
                                        <input
                                            v-model="form.paper_size"
                                            ...
                                            class="text-main focus:ring-main"
                                        />
                                        <span>80 mm (Lebar)</span>
                                    </div>
                                    <span class="text-xs text-slate-400 font-normal">~48 Karakter</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                            <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                                <div>
                                    <div class="font-medium text-sm text-slate-700">Auto Print Struk</div>
                                    <div class="text-xs text-slate-500">Cetak otomatis setelah bayar</div>
                                </div>
                                <Switch id="auto_print" v-model="form.auto_print" size="md" />
                            </div>
                            <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                                <div>
                                    <div class="font-medium text-sm text-slate-700">Salinan Dapur</div>
                                    <div class="text-xs text-slate-500">Cetak tiket pesanan dapur</div>
                                </div>
                                <Switch id="print_kitchen_copy" v-model="form.print_kitchen_copy" size="md" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-xs p-5">
                    ...
                    <div class="flex flex-col gap-4">
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div>
                                <div class="font-medium text-sm text-slate-700">Tampilkan Logo Toko</div>
                                <div class="text-xs text-slate-500">Cetak logo di bagian atas struk</div>
                            </div>
                        ...
                        <TextField
                            ...
                            :error="form.errors.custom_header_title"
                        />
                        ...
                        <TextField
                            ...
                            :error="form.errors.header_notes"
                        />

                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                                <div class="font-medium text-xs text-slate-700">Tampilkan Alamat</div>
                                <Switch id="show_address" v-model="form.show_address" size="sm" />
                            </div>
                            <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                                <div class="font-medium text-xs text-slate-700">Tampilkan No. Telp</div>
                                <Switch id="show_phone" v-model="form.show_phone" size="sm" />
                            </div>
                            <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                                <div class="font-medium text-xs text-slate-700">Tampilkan Email</div>
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-xs p-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div class="font-medium text-xs text-slate-700">Nama Kasir Bertugas</div>
                            <Switch id="show_cashier_name" v-model="form.show_cashier_name" size="sm" />
                        </div>
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div class="font-medium text-xs text-slate-700">Nama Pelanggan</div>
                            <Switch id="show_customer_name" v-model="form.show_customer_name" size="sm" />
                        </div>
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div class="font-medium text-xs text-slate-700">Tipe Pesanan (Dine-in/Takeaway)</div>
                            <Switch id="show_order_type" v-model="form.show_order_type" size="sm" />
                        </div>
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div class="font-medium text-xs text-slate-700">Detail Modifier / Topping</div>
                            <Switch id="show_modifiers" v-model="form.show_modifiers" size="sm" />
                        </div>
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div class="font-medium text-xs text-slate-700">Catatan Khusus Produk</div>
                            <Switch id="show_item_notes" v-model="form.show_item_notes" size="sm" />
                        </div>
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div class="font-medium text-xs text-slate-700">Rincian Pajak (PPN/PB1)</div>
                            <Switch id="show_tax_detail" v-model="form.show_tax_detail" size="sm" />
                        </div>
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg sm:col-span-2">
                            <div class="font-medium text-xs text-slate-700">Rincian Biaya Layanan (Service Charge)</div>
                            <Switch id="show_service_charge" v-model="form.show_service_charge" size="sm" />
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-xs p-5">
                    ...
                    <div class="flex flex-col gap-4">
                        <TextareaField
                            ...
                            rows="2"
                            :error="form.errors.footer_notes"
                        />
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <TextField
                                ...
                                :error="form.errors.social_media_info"
                            />
                            <TextField
                                id="wifi_info"
                                ...
                                :error="form.errors.wifi_info"
                            />
                        </div>

                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div>
                                <div class="font-medium text-sm text-slate-700">Tampilkan QR Code Struk</div>
                                <div class="text-xs text-slate-500">QR untuk feedback atau cek transaksi digital</div>
                            </div>
            ...
            <div class="lg:col-span-5">
                <div class="sticky top-20 flex flex-col items-center">
                    <div class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                    ...
                    >
                        ...
                        <div class="border-b-2 border-dashed border-slate-300 pb-3 mb-3 text-center">
                            <!-- Logo -->
                            <div v-if="form.show_logo" class="flex justify-center mb-2">
                                <div class="w-12 h-12 rounded-full bg-slate-200 border border-slate-300 flex items-center justify-center text-slate-500 text-sm font-bold">
                                    LOGO
                                </div>
                            </div>

                            <!-- Header Title -->
                            <div class="font-bold text-sm uppercase text-slate-900 tracking-wide">
                                {{ form.custom_header_title || selectedOutlet?.name || 'NAMA OUTLET KASIR' }}
                            </div>

                            <!-- Address & Contact -->
                            <div v-if="form.show_address && selectedOutlet?.address" class="text-[11px] text-slate-600 mt-1">
                                {{ selectedOutlet.address }}
                            </div>
                            <div v-if="form.show_phone && selectedOutlet?.phone" class="text-[11px] text-slate-600">
                                Telp: {{ selectedOutlet.phone }}
                            </div>
                            <div v-if="form.show_email && selectedOutlet?.email" class="text-[11px] text-slate-600">
                                {{ selectedOutlet.email }}
                            </div>

                            <!-- Header Note -->
                            <div v-if="form.header_notes" class="text-[11px] italic text-slate-600 mt-1.5">
                                "{{ form.header_notes }}"
                            </div>
                        </div>

                        <!-- Meta Info -->
                        <div class="border-b border-dashed border-slate-300 pb-2 mb-2 text-[11px] text-slate-700">
                        ...
                        <div class="border-b border-dashed border-slate-300 pb-2 mb-2 text-[11px]">
                            ...
                            <div v-if="form.show_modifiers" class="text-[10px] text-slate-500 pl-3">
                                + Less Sugar, Extra Shot (+Rp 5.000)
                            </div>
                            <div v-if="form.show_item_notes" class="text-[10px] italic text-slate-500 pl-3">
                                Catatan: Sedikit es
                            </div>

                            <!-- Item 2 -->
                            <div class="flex justify-between font-medium mt-1.5">
                        ...
                        <div class="border-b border-dashed border-slate-300 pb-2 mb-2 text-[11px]">
                            ...
                            <div v-if="form.show_tax_detail" class="flex justify-between text-slate-600">
                                <span>PB1 / Pajak (10%)</span>
                                <span>Rp 7.000</span>
                            </div>
                            <div v-if="form.show_service_charge" class="flex justify-between text-slate-600">
                                <span>Service Fee (5%)</span>
                                <span>Rp 3.500</span>
                            </div>
                            <div class="flex justify-between font-bold text-xs text-slate-900 pt-1 mt-1 border-t border-slate-200">
                                <span>TOTAL</span>
                                <span>Rp 80.500</span>
                            </div>
                        ...
                        <div class="text-center pt-1 text-[11px] text-slate-600">
                            <div v-if="form.footer_notes" class="whitespace-pre-line mb-1.5 font-medium">
                                {{ form.footer_notes }}
                            </div>
                            <div v-if="form.social_media_info" class="text-[10px] text-slate-500">
                                {{ form.social_media_info }}
                            </div>
                            <div v-if="form.wifi_info" class="text-[10px] text-slate-500">
                                {{ form.wifi_info }}
                            </div>

                            <!-- QR Code Preview -->
                            <div v-if="form.show_qr_code" class="flex flex-col items-center justify-center mt-2.5 pt-2 border-t border-slate-200">
                                <div class="w-16 h-16 bg-slate-900 text-white flex items-center justify-center rounded-sm text-[8px] font-bold">
                                    [QR CODE]
                                </div>
                                <span class="text-[9px] text-slate-400 mt-1">Scan untuk detail transaksi</span>
                            </div>
                        </div>
                    </div>
                </div>
