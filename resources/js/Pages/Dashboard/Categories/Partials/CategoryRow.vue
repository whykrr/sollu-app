...
const confirmDelete = (type, category) => {
    ...
    if (confirm(message)) {
        const routeName = type === 'soft' ? 'categories.destroy' : 'categories.force-delete';
        Inertia.delete(route(routeName, category.id), { preserveScroll: true });
    }
};
    ...
    <tr :class="{ 'bg-gray-50': level > 0 }">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div v-if="category.merchant_id" class="flex items-center justify-end space-x-2">
                <Link :href="route('categories.edit', category.id)" class="text-indigo-600 hover:text-indigo-900">Edit</Link>
                <button @click="confirmDelete('soft', category)" class="text-red-600 hover:text-red-900">Sampah</button>
                <button @click="confirmDelete('hard', category)" class="text-red-600 hover:text-red-900 font-bold">Hapus</button>
            </div>
            <div v-else>
