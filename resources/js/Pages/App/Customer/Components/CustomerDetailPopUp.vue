<template>
    <div class="space-y-4 flex-1 overflow-y-auto">
        ...
        >
            ...
            <div v-if="detail?.is_active !== undefined">
                <span v-if="detail.is_active" class="badge badge-success px-3 py-1">Aktif</span>
                <span v-else class="badge badge-neutral-500 px-3 py-1">Tidak Aktif</span>
            </div>
        </div>

        <!-- Detail Info Grid -->
        <div class="bg-white p-6 rounded-xl border border-slate-200">
            <h4 class="font-semibold text-slate-700 mb-4">Informasi Detail</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                    <span class="text-slate-500 block text-xs">Tanggal Lahir / Umur</span>
                    <span class="font-medium text-slate-800">
                        {{ detail?.birthdate || '-' }}
                        <span v-if="detail?.age" class="text-slate-500 font-normal"
                            >({{ detail.age }} tahun)</span
                        >
                    </span>
                </div>
                <div>
                    <span class="text-slate-500 block text-xs">Jenis Kelamin</span>
                    <span class="font-medium text-slate-800 capitalize">{{ genderLabel }}</span>
                </div>
                <div class="md:col-span-2">
                    <span class="text-slate-500 block text-xs">Alamat Lengkap</span>
                    <span class="font-medium text-slate-800">{{ detail?.address || '-' }}</span>
                </div>
                <div class="md:col-span-2">
                    <span class="text-slate-500 block text-xs">Catatan Khusus</span>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-white p-4 rounded-xl border border-slate-200">
                <div class="text-slate-500 text-xs mb-1">Transaksi Berulang</div>
                <div class="font-bold text-lg text-slate-800">
                    {{ detail?.summary?.total_transactions || 0 }} kali
                </div>
            </div>
            <div class="bg-white p-4 rounded-xl border border-slate-200">
                <div class="text-slate-500 text-xs mb-1">Total Belanja</div>
                <div class="font-bold text-lg text-slate-800">
                    {{ formatCurrency(detail?.summary?.total_spent) }}
                </div>
            </div>
            <div class="bg-white p-4 rounded-xl border border-slate-200">
                <div class="text-slate-500 text-xs mb-1">Rata-rata Belanja</div>
                <div class="font-bold text-lg text-slate-800">
                    {{ formatCurrency(detail?.summary?.average_spent) }}
                </div>
            </div>
            <div class="bg-white p-4 rounded-xl border border-slate-200">
                <div class="text-slate-500 text-xs mb-1">Transaksi Terakhir</div>
                <div class="font-bold text-lg text-slate-800">
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div class="p-4 border-b border-slate-200 bg-slate-50/50">
                <h4 class="font-semibold text-slate-700">Riwayat Transaksi Terakhir</h4>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-600">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-if="!detail?.recent_transactions?.length">
    ...
    <Teleport v-if="isMounted" to="#popUpFooter">
        <div class="flex items-center justify-end w-full gap-2">
            ...
            <button type="button" class="btn btn-outline-primary" @click="handleEdit">
                Ubah Data
            </button>
        </div>
    </Teleport>
