...
const getInitialMinStock = () => {
    if (props.product?.inventory_items?.length > 0) {
        const item = props.product.inventory_items.find((item) => item.min_stock !== null && item.min_stock !== undefined);
...
const getComboKey = (comboOptions) => {
    return Object.keys(comboOptions).sort().map((k) => `${k}:${comboOptions[k]}`).join('|');
};

const generateCombinations = (groups) => {
    const activeGroups = groups.filter((g) => g.name.trim() !== '' && g.options.some((o) => o.name.trim() !== ''));
    ...
    const helper = (groupIndex, currentCombo) => {
        ...
        group.options.forEach((opt) => {
            if (opt.name.trim() !== '') {
                helper(groupIndex + 1, { ...currentCombo, [group.name]: opt.name });
            }
        });
    };
    helper(0, {});
...
const generateSku = (comboOptions) => {
    ...
    const suffix = Object.values(comboOptions).map((v) => v.toUpperCase().replace(/\s+/g, '')).join('-');
...
const updateCombinations = () => {
    ...
    form.variant_combinations = newCombos.map((options) => {
        ...
        if (!variantOutletPriceMap.value[key]) variantOutletPriceMap.value[key] = {};
        return {
            options: options,
...
if (isEdit.value && props.product) {
    ...
    outlets.forEach((o) => {
        const p = props.product.prices.find((pr) => pr.outlet_id === o.id && !pr.inventory_item_id);
        if (p) {
    ...
    if (props.product.variant_groups && props.product.variant_groups.length > 0) {
        ...
        if (props.product.inventory_items) {
            const variantItems = props.product.inventory_items.filter((item) => item.item_type === 'variant_sku');
            if (variantItems.length > 0) {
                ...
                form.variant_combinations = variantItems.map((invItem) => {
                    ...
                    invItem.variant_group_options.forEach((opt) => {
                        const vg = props.product.variant_groups.find((g) => g.id === opt.variant_group_id);
                        const gName = vg ? vg.name : '';
                        if (gName) combinationOptions[gName] = opt.name;
                    });
                    const priceObj = props.product.prices.find((p) => p.inventory_item_id === invItem.id && !p.outlet_id);
                    const price = priceObj ? priceObj.amount : form.base_price;
                    if (priceObj && Number(priceObj.amount) !== Number(form.base_price)) hasCustomPrices = true;
                    const comboKey = getComboKey(combinationOptions);
                    variantOutletPriceMap.value[comboKey] = {};
                    
                    const itemImage = props.product.images?.find(img => img.inventory_item_id === invItem.id);

                    outlets.forEach((outlet) => {
                        const op = props.product.prices.find((p) => p.inventory_item_id === invItem.id && p.outlet_id === outlet.id);
                        if (op) {
    ...
    outlets.forEach((o) => { outletStatusMap.value[o.id] = true; });
    form.variants = [{ name: '', options: [{ name: '' }] }];
}
...
watch(() => form.variants, () => { updateCombinations(); }, { deep: true });
watch(() => form.track_inventory, (track) => { if (!track) form.uom_id = ''; });
watch(() => form.base_price, (newVal) => {
    form.variant_combinations.forEach((combo) => {
...
const steps = computed(() => {
    ...
    if (form.product_type === 'basic' && form.has_variant) {
        s.push({ id: 'variant', component: StepVariantSetup, title: 'Atur Varian' });
    }
...
watch(() => steps.value, () => {
    if (props.targetStepId) {
        const idx = steps.value.findIndex(s => s.id === props.targetStepId);
        if (idx !== -1) currentStepIndex.value = idx;
    }
}, { immediate: true });
...
const nextStep = () => { if (!isLastStep.value) currentStepIndex.value++; };
const prevStep = () => { if (!isFirstStep.value) currentStepIndex.value--; };
    ...
    if (!form.has_variant) {
        ...
        if (customizeVariantPrices.value) {
            form.variant_combinations.forEach((combo) => {
                ...
                combo.outlet_prices = Object.keys(map).filter((oId) => map[oId] !== undefined && map[oId] !== null && map[oId] !== '').map((oId) => ({ outlet_id: oId, amount: map[oId] }));
            });
        } else {
            form.variant_combinations.forEach((combo) => {
