        ...
        <template #header>
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-xl shadow-sm border border-neutral-200/60 gap-4 mb-4">
                <div class="flex items-center gap-4">
                    <Link :href="route('cockpit.merchants.index')" class="text-neutral-400 hover:text-neutral-600 transition-colors mr-2">
                    ...
                    <div>
                        ...
                        <div class="text-sm text-neutral-500 mt-1">ID: <span class="font-mono">{{ business.id.substring(0, 8) }}</span> • Joined: {{ new Date(business.created_at).toLocaleDateString() }}</div>
                    </div>
                </div>
                <div class="flex gap-2">
                    ...
                    >
                        <FontAwesomeIcon :icon="faBan" class="mr-2" />Suspend Merchant
                    </button>
                    <button 
                        v-else
                    ...
                    >
                        <FontAwesomeIcon :icon="faCheck" class="mr-2" />Activate Merchant
                    </button>
                </div>
            </div>

            <div class="bg-white rounded-xl shadow-sm border border-neutral-200/60">
                <div class="border-b border-neutral-200 px-4">
                    <div class="flex gap-6 overflow-x-auto hide-scrollbar">
                        <button class="px-1 py-3 text-sm font-medium border-b-2 border-main text-main">Overview</button>
                        <button class="px-1 py-3 text-sm font-medium border-b-2 border-transparent text-neutral-500 hover:text-neutral-800 cursor-not-allowed opacity-50">Subscription</button>
                        <button class="px-1 py-3 text-sm font-medium border-b-2 border-transparent text-neutral-500 hover:text-neutral-800 cursor-not-allowed opacity-50">Outlets ({{ business.outlets_count }})</button>
                        <button class="px-1 py-3 text-sm font-medium border-b-2 border-transparent text-neutral-500 hover:text-neutral-800 cursor-not-allowed opacity-50">Users ({{ business.users_count }})</button>
                    </div>
                </div>
            </div>
        </template>
...
const toggleStatus = (newStatus) => {
    if (confirm(`Are you sure you want to change this merchant's status to ${newStatus}?`)) {
        router.post(route('cockpit.merchants.toggle-status', props.business.id), {
            status: newStatus
        }, {
            preserveScroll: true
        });
