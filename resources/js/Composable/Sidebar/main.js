import { faBox, faBoxes, faCartShopping, faChartLine, faChartPie, faHashtag, faReceipt, faUsers, faUserTie, faWallet } from '@fortawesome/free-solid-svg-icons'

export const mainSidebars = [
    {
        type: 'item',
        url: route('overview'),
        icon: faChartPie,
        label: 'Ringkasan',
        permissions: [],
        activeRoute: 'overview',
    },
    {
        type: 'item',
        route: '#',
        icon: faReceipt,
        label: 'Penjualan',
        permissions: [],
        activeRoute: 'sales',
    },
    {
        type: 'dropdown',
        url: '#',
        icon: faBox,
        label: 'Master Produk',
        permissions: [],
        activeRoute: 'master.',
        items: [
            {
                type: 'item',
                url: route('master.categories.index'),
                label: 'Kategori Produk',
                permissions: [],
                activeRoute: 'master.categories.',
            },
            {
                type: 'item',
                url: route('master.products.index'),
                label: 'Produk',
                permissions: [],
                activeRoute: 'master.products.',
            },
            {
                type: 'item',
                url: route('master.modifiers.index'),
                label: 'Opsi Tambahan',
                permissions: [],
                activeRoute: 'master.modifiers.',
            },
        ],
    },
    {
        type: 'dropdown',
        url: '#',
        icon: faBoxes,
        label: 'Inventori',
        permissions: [],
        activeRoute: 'inventories.',
        items: [
            {
                type: 'item',
                url: route('inventories.raw.index'),
                label: 'Bahan Baku',
                permissions: [],
                activeRoute: 'inventories.raw.',
            },
            {
                type: 'item',
                url: route('inventories.stocks.index'),
                label: 'Stok',
                permissions: [],
                activeRoute: 'inventories.stocks.',
            },
            {
                type: 'item',
                url: route('inventories.suppliers.index'),
                label: 'Pemasok / Supplier',
                permissions: [],
                activeRoute: 'inventories.suppliers.',
            },
            {
                type: 'item',
                url: route('inventories.purchases.index'),
                label: 'Pembelian (PO)',
                permissions: [],
                activeRoute: 'inventories.purchases.',
            },
            {
                type: 'item',
                url: route('inventories.stocktaking.index'),
                label: 'Stok Opname',
                permissions: [],
                activeRoute: 'inventories.stock-taking.',
            },
            {
                type: 'item',
                url: route('inventories.adjustments.index'),
                label: 'Penyesuaian Stok',
                permissions: [],
                activeRoute: 'inventories.adjustments.',
            },
            {
                type: 'item',
                url: route('inventories.transfers.index'),
                label: 'Mutasi Stok',
                permissions: [],
                activeRoute: 'inventories.transfers.',
            },
        ],
    },
    {
        type: 'item',
        url: '#',
        icon: faCartShopping,
        label: 'Pesanan / Order',
        permissions: [],
        activeRoute: 'orders',
    },
    {
        type: 'item',
        url: '#',
        icon: faUsers,
        label: 'Pelanggan',
        permissions: [],
        activeRoute: 'members',
    },
    {
        type: 'item',
        url: route('employees.index'),
        icon: faUserTie,
        label: 'Pegawai',
        permissions: ['user.view'],
        activeRoute: 'employees',
    },
    {
        type: 'dropdown',
        url: '#',
        icon: faChartLine,
        label: 'Laporan',
        permissions: [],
        activeRoute: 'reports',
        items: [
            {
                type: 'item',
                route: '#',
                label: 'Penjualan Produk',
                permissions: [],
                activeRoute: 'reports.products.',
            },
            {
                type: 'item',
                route: '#',
                label: 'Stok',
                permissions: [],
                activeRoute: 'reports.stock.',
            },
            {
                type: 'item',
                route: '#',
                label: 'Pegawai',
                permissions: [],
                activeRoute: 'reports.employees.',
            },
            {
                type: 'item',
                route: '#',
                label: 'Retur',
                permissions: [],
                activeRoute: 'reports.return.',
            },
            {
                type: 'item',
                route: '#',
                label: 'Pajak & Diskon',
                permissions: [],
                activeRoute: 'reports.tax-discounts.',
            },
            {
                type: 'item',
                route: '#',
                label: 'Omset',
                permissions: [],
                activeRoute: 'reports.revenue.',
            },
        ],
    },
    {
        type: 'dropdown',
        url: '#',
        icon: faHashtag,
        label: 'Template',
        permissions: [],
        activeRoute: 'template',
        items: [
            {
                type: 'item',
                url: route('template.form'),
                label: 'Form',
                permissions: [],
                activeRoute: 'template.form',
            },
            {
                type: 'item',
                url: route('template.cards'),
                label: 'Card',
                permissions: [],
                activeRoute: 'template.cards',
            },
            {
                type: 'item',
                url: route('template.navigation'),
                label: 'Navigation & Tab',
                permissions: [],
                activeRoute: 'template.navigation',
            },
            {
                type: 'item',
                url: route('template.buttons'),
                label: 'Buttons',
                permissions: [],
                activeRoute: 'template.buttons',
            },
            {
                type: 'item',
                url: route('template.charts'),
                label: 'Charts',
                permissions: [],
                activeRoute: 'template.charts',
            },
            {
                type: 'item',
                url: route('template.notifications'),
                label: 'Notifications',
                permissions: [],
                activeRoute: 'template.notifications',
            },
            {
                type: 'item',
                url: route('template.widgets'),
                label: 'Widgets',
                permissions: [],
                activeRoute: 'template.widgets',
            },
        ],
    },
]
